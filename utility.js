...
const sheetsAgent = new https.Agent({
    ...
    keepAliveMsecs: 3000,        // 3秒发一次空包维持通道通畅，这是怎么做到的？ // 发送空包会有什么副作用吗？
...
/**
 * 将二维数组转换为 HTML 表格（刚性保证空行、空格、换行符及单元格样式）
...
export function ConvertRowsToHtmlTable(rows) {
    ...
    // 辅助处理函数：对单元格内容进行安全转义与空行填充
    const formatCellContent = (val) => {
        // 1. 完全无数据（undefined, null, ''）：用 &nbsp; 撑开单元格
        ...
        // 将连续换行（即中间什么都没有的空行）替换为 &nbsp;<br/> 强行占据物理高度
        // 避免 white-space 渲染时塌陷
        str = str.replace(/\n\n/g, '\n&nbsp;\n');

        // 将换行符转为 <br/>
        str = str.replaceAll('\n', '<br/>');

        // 4. 对多个连续空格进行不换行空格替换，防止空格被浏览器合并
        str = str.replaceAll('  ', ' &nbsp;');
    ...
    let html = '<table style="border-collapse: collapse; border: 1px solid; text-align: left; white-space: pre; font-family: monospace; font-size: 1em; width: auto; max-width: 100%;">';
    ...
    if (rows[0] && Array.isArray(rows[0])) {
        ...
        rows[0].forEach(header => {
            ...
            html += `<th style="border-collapse: collapse; border: 1px solid; text-align: left; white-space: pre; font-family: monospace; font-size: 1.1em; padding: 0.3em 0.5em; background-color: #f2f2f2;">${cleanHeader}</th>`;
        });
        html += '</tr></thead>';
    }
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        row.forEach(cell => {
            ...
            // 💡 使用 pre-wrap 配合 &nbsp; 充填，物理撑开每一行
            html += `<td style="border-collapse: collapse; border: 1px solid; text-align: left; white-space: pre; font-family: monospace; font-size: 1em; padding: 0.3em 0.5em;">${cleanCell}</td>`;
        });
        html += '</tr>';
    }
    html += '</tbody></table>';
