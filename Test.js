import { GetGS, GetSpreadsheetID, SendTG, Sleep, UpdateGS, ClearGS, ObjToA2dNumBoolStr, GetSheetsIDfromSheet, makeRequestBodyArrayofBatchUpdate_deleteLines, makeRequestBodyArrayofBatchUpdate_append, makeRequestBodyArrayofBatchUpdate_clearUpdate, BatchUpdateGS, isStrictString } from "./utility.js"
...
async function sendOK(spreadsheetID, chat_id) {
...
async function TestBatchUpdate(spreadsheetID, chat_id) {
    ...
    const clearUpdateBody = {
        sheetID: sheetIDData.test4,
        range: 'test4!A29:29' ,
        values: [[1,2,3,4,5,6,7]]
    } ;

    const request_clearupdate = makeRequestBodyArrayofBatchUpdate_clearUpdate(clearUpdateBody) ;
...
async function testGeminiAI(spreadsheetID, chat_id) {
    const { AskGemini } = await import("./geminiAI.js");

    const question = '你能否实时分析某个关键词，例如BTC，在全网的热度，并分析是热度积极方向上升还是消极方向上升' ;

    let response = null;
    try { response = await AskGemini(question) }
    catch (e) { response = `调用AI出错：${e.message}` }

    SendTG('来自AI回复', response, chat_id) ;
}
