...
export const TradeBot = {
    ...
    async CreateBasicAttr(tvData, thisLogs) {
        ...
        if (TradeBot[this.tbName_spreadsheetID] === null) {
            ...
            catch (e) { return this.returnRunningWellErrMessage('获取spreadsheetID失败', e.message) }
        }
        ...
        if (isEmptyObject(TradeBot[this.tbName_sheetsID])) {
            ...
            catch (e) { return this.returnRunningWellErrMessage('获取sheetsID失败', e.message) }
        }
        ...
        if (isEmptyObject(TradeBot[this.tbName_gsData]) ) {
            ...
            if (!isStrictTrue(r_Get_gsData) || isStrictString(r_Get_gsData)) { return this.returnRunningWellErrMessage('Get_gsData() 失败', r_Get_gsData ?? '') }
            ...
            if (this.TradeBotNumber !== this.mainData.TradeBotNumber) { return this.returnRunningWellErrMessage('直接从缓存获取gsData失败','this.TradeBotNumber !== this.mainData.TradeBotNumber') }
        ...
        if (currentLock !== CV.noLOCK && !toResetGSLOCK) {
            ...
            return this.returnRunningWellErrMessage('检查gsLOCK失败', '上一次运行大TradeBot锁被释放的情况下, GS锁未被释放'); 
        }

        // 发送设置GSLOCK任务, 写入不成功是小概率事件, 不必等待结果, 只在运行到重要情况前确认
        if (currentLock === CV.noLOCK || toResetGSLOCK) {
            // await UpdateGS(this.spreadsheetID, toGCPData.lockRange, [[this.lockName]]);
            thisLogs.AddNewLogLine('去GS设锁， 不等待，后续必要时检查设锁状态')
            this.task_setGSLOCK = try3times(UpdateGS, this.spreadsheetID, this.toGCPData.lockRange, [[this.lockName]])
                .catch((e) => {
                    ...
                    this.thisLogs.AddNewLogLine('设GS锁失败: ' + e.message);
                    const r_releaseTradeBotLOCK = this.releaseTradeBotLOCK();
                    if (!isStrictTrue(r_releaseTradeBotLOCK)) { this.thisLogs.AddNewErrLogLine('TradeBot锁释放失败') }
                    else { this.thisLogs.AddNewLogLine('TradeBot锁已释放') }
                })
                .finally(() => {
                    if(!this.task_gslock_fail) {
                        this.task_gslock_isOK = true ;
                        this.thisLogs.AddNewLogLine('设GS锁成功') ;
                    }
                }) ;
        }
     ...
     */
    releaseTradeBotLOCK() {
        const tbName_lastLockTime   = this.getThisTvMainData('tbName_lastLockTime') ;
        const tbName_isLocked       = this.getThisTvMainData('tbName_isLocked')     ;
        const LockTime              = this.getThisTvMainData('LockTime')            ;

        if (TradeBot[tbName_lastLockTime] !== LockTime) { return '释放大锁失败, 此信号无权解锁' }
        else { TradeBot[tbName_isLocked] = false; return true; }
    } ,
    ...
    returnRunningWellErrMessage(errTitle, errContent) {
        const errMessage = `${errTitle}: ${errContent}`.trim() ;
    ...
    async ToCheckInitiate() {
        ...
        } catch (e) { return this.returnRunningWellErrMessage('ToCheckInitiate()失败', e.message) }
    } ,
    ...
    async CheckAllPosition_withBroker() {
        ...
        try {
            ...
            return true ;
        } catch (e) { return this.returnRunningWellErrMessage('CheckAllPosition_withBroker()失败', e.message) }
    } ,
    ...
    async ToCheckFundFee() {
        try {
            ...
            return true;
        } catch (e) { return this.returnRunningWellErrMessage('ToCheckFundFee()失败', e.message) }
    ...
    */
    async ToSell() {
        ...
        try {
            ...
            ToWebListAddNewLine({type: 'trade', content: newWebLine});
    ...
    */
    async ToBuy() {
        ...
        } catch (e) { return this.returnRunningWellErrMessage('ToBuy()失败', e.message) }
    } ,
    ...
    async ToCheckWaitingOrder() {
        try {
            ...
            return true;
        } catch (e) { return this.returnRunningWellErrMessage('ToCheckWaitingOrder()失败', e.message) }
    ...
    async ToSendAlertMessage() {
        try {
            ...
            let toReadA2d, toEmailA2d ;
            for (const [i, v] of getDataList.entries()) {
                if (v.name === 'toReadA2d' ) {toReadA2d  = valuesArray[i] ; continue ;} 
                if (v.name === 'toEmailA2d') {toEmailA2d = valuesArray[i] ; continue ;} 
            }

            this.sendToTG(toReadA2d).catch(() => { });
            this.sendToEmail(toEmailA2d).catch(() => { });
...
export async function HandleTradeBot(tvData, thisLogs) {
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'CreateTradeBot' ; thisTime.UpdateTime() ;}
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'ToCheckInitiate()' ; thisTime.UpdateTime() ;}
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'CheckAllPosition_withBroker()' ; thisTime.UpdateTime() ;}


    bot.CalcuBuySellLimit();
    thisLogs.AddNewLogLine('CalcuBuySellLimit() success');
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'CalcuBuySellLimit()' ; thisTime.UpdateTime() ;}
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'ToCheckFundFee()' ; thisTime.UpdateTime() ;}
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'ToSell()' ; thisTime.UpdateTime() ;}
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'ToBuy()' ; thisTime.UpdateTime() ;}
    ...
    if(thisTime.HowLongToNOW() > theLongestTime) { theLongestTime = thisTime.HowLongToNOW() ; theLongestTimeTask = 'ToCheckWaitingOrder()' ; thisTime.UpdateTime() ;}
